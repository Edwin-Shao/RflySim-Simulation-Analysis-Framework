...
class PX4MavCtrler:
    ...
    def __init__(self, CopterID=1, ip="127.0.0.1", Com="udp", port=0):
        ...
        if (
            Com == "udp" or Com == "UDP" or Com == "Udp"
        ) and CopterID > 10000:  # 如果是UDP通信模式
        ...
        self.uavGlobalPos = [
            ...
            0,
            0,
        ]  # Estimated global position from PX4 that transferred to UE4 map
        ...
        if is_use_ros1:
            ...
            self.vel_raw_pub = rospy.Publisher(
            ...
            self.setparamService = rospy.ServiceProxy(
                "/" + self.rosName + "/param/set", ParamSet
            )
            self.sendCmdLongService = rospy.ServiceProxy(
            ...
            self.local_pose_sub = self.ros_node.create_subscription(
                PoseStamped,
                "/" + self.rosName + "/local_position/pose",
                self.local_pose_callback,
                SENSOR_QOS,
            )
            self.local_vel_sub = self.ros_node.create_subscription(
                TwistStamped,
                "/" + self.rosName + "/local_position/velocity_local",
                self.local_vel_callback,
                SENSOR_QOS,
            )
            self.mavros_sub = self.ros_node.create_subscription(
                State,
                "/" + self.rosName + "/state",
            ...
            self.imu_sub = self.ros_node.create_subscription(
                Imu, "/" + self.rosName + "/imu/data", self.imu_callback, SENSOR_QOS
            )

            """
            ros publishers
            """
            self.vel_pub = self.ros_node.create_publisher(
                TwistStamped, "/" + self.rosName + "/setpoint_velocity/cmd_vel", 10
            )
            self.vel_raw_pub = self.ros_node.create_publisher(
                PositionTarget, "/" + self.rosName + "/setpoint_raw/local", 10
            )
            self.mav_raw_pub = self.ros_node.create_publisher(
                Mavlink, "/uas" + str(self.CopterID) + "/mavlink_sink", 10
            )

            """
            ros services
            """
            self.armService = self.ros_node.create_client(
                CommandBool, "/" + self.rosName + "/cmd/arming"
            )
            self.flightModeService = self.ros_node.create_client(
                SetMode, "/" + self.rosName + "/set_mode"
            )
            self.setparamService = self.ros_node.create_client(
                ParamSet, "/" + self.rosName + "/param/set"
            )
            self.sendCmdLongService = self.ros_node.create_client(
                CommandLong, "/" + self.rosName + "/cmd/command"
            )
            print("Px4 Controller Initialized!")
    ...
    ## @brief ROS守护进程spin函数,保持节点运行
    # @param  无
    # @return  无
    def spin(self):
        global is_use_ros1, isRosOk
        if not isRosOk:
            print("ROS not found!")
            return
        if is_use_ros1:
    ...
    def InitMavLoop(self):
        if self.isCom:
            ...
            if is_use_ros1:
                cmdStr = (
                    "roslaunch mavros px4.launch tgt_system:="
                    + str(self.tgtSys)
                    + ' fcu_url:="serial://'
                    + self.port
                    + ":"
                ...
                cmdStr = (
                    "ros2 launch mavros px4.launch tgt_system:="
                    + str(self.tgtSys)
                    + ' fcu_url:="serial://'
                    + self.port
                    + ":"
        ...
        print(cmdStr)
        self.child = subprocess.Popen(cmdStr, shell=True, stdout=subprocess.PIPE)
    ...
    def waitForStartMsg(self):
        ...
        try:
            if os.name == 'posix':
                sock.setsockopt(socket.SOL_SOCKET, 15, 1)  # 15 == SO_REUSEPORT（Linux）
        except OSError:
            pass
    ...
    #  @param yawrate 偏航角速率
    def SendPosVelNED(
        self, PosE=[math.nan] * 3, VelE=[math.nan] * 3, yaw=math.nan, yawrate=math.nan
    ):
    ...
    def SendHILCtrlMsg(self, ctrls=[0] * 16, idx=0):
        ...
        # 打包一个mavlink的包
        msg = self.mav0.hil_actuator_controls_encode(
            time_boot_ms, controls, int(mode), int(flag)
        )
        msg.pack(self.mav0)
